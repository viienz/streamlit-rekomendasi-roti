...
import pandas as pd
import numpy as np
from mlxtend.freqpuent_patterns import association_rules, apriori
...
df["month"].replace([i for i in range(1, 12 + 1)], ["January","February","March","April","Mei","Juni","Juli","Agustus","September","Oktober", "November", "Desember", ], inplace=True)
df["day"].replace([i for i in range(6 +1)], ["Monday", "Tuesday", "Wednesday", "Thursday","Friday","Saturday","Sunday"],inplace=True)

st.title("Market Analysis Food")
...
if type(data) != type("No Result!!"):
    ...
    item_count_pivot = item_count_pivot.applymap(endcode)
    ...
    rules.short_values('confidence', ascending = False, inplace = True)
...
def return_item_df(item_antencendent):
    data = rules[['antecedents', 'consequents']].copy()
    
    data["antecedents"] = data["antecedents"].apply(parce_list)
    data["consequents"] = data["consequents"].apply(parce_list)
    
    return list(data.loc[data['antecedents'] == item_antencendent].iloc[0,:1])

if type(data) != type("No Result!!"):
    st.markdown("Hasil Rekomendasi : ")
    st.success(f"Jika Konsumen Membeli **{item}**, maka Konsumen Membeli **{return_item_df(item)[1]}** secara bersamaan.")
